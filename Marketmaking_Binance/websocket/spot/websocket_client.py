...
import pandas as pd
base_url = pd.read_csv(os.path.dirname(os.path.abspath(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))) + "/config.csv")['websocket'][0]

print(base_url)
from Spot_trading.websocket.websocket_client import BinanceWebsocketClient
class SpotWebsocketClient(BinanceWebsocketClient):
    def __init__(self, stream_url=base_url):
    # def __init__(self, stream_url= "wss://testnet.binance.vision/ws"):
