...
class MarketMaker():
            ...
            if mes['e'] == 'executionReport':
                if mes['s'] == self.symbol:
                    ...
                    if mes['x'] == "TRADE":
                        if mes['S'] == "BUY":
                            print("@@@@@@@@@@@@@@@@111111@@@@@@@@@@@@@@")
                            if round(float(mes['p']),asset_info[self.symbol]['minPrice']) == self.df.iloc[num_orders*2, 1] and round(float(mes['z']),asset_info[self.symbol]['minQtydigits']) == self.df.iloc[num_orders*2, 4]:
                                print("@@@@@@@@@@@@@@@@222222@@@@@@@@@@@@@@")
                                ...
                                try:
                                    ...
                                    self.sell_amt += order_quantity
                                    print("ERROR")
                        if mes['S'] == "SELL":
                            print("@@@@@@@@@@@@@@@@444444@@@@@@@@@@@@@@")

                            if round(float(mes['p']),asset_info[self.symbol]['minPrice']) == self.df.iloc[0,1] and round(float(mes['z']),asset_info[self.symbol]['minQtydigits']) == self.df.iloc[0,4]:
                                print("@@@@@@@@@@@@@@@@55555555@@@@@@@@@@@@@@")
                                ...
                                try:
                                    ...
                                    print(order_price)
                                    a = self.client.new_order(symbol=self.symbol, type='limit',side='buy', quantity=order_quantity, price=order_price)
                                    print(a)
                                    self.buy_amt = 0
                                    self.min_order += 1
                                    self.max_order += 1
    ...
    def rebalancing(self):
        ...
        self.balance_array[self.symbol]['free'] = self.balance
        self.balance_array[self.symbol]['locked'] = 0

        print("OPTIMISED QTY:",self.balance_array)
        print("Actual QTY:", print(self.asset_balance()))
